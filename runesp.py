...
if not args.no_viewer and first_mesh_path and os.path.exists(first_mesh_path):
    ...
    try:
        ...
        # Left: ESP32-CAM photo
        if has_input:
            pl.subplot(0, 0)
            img_data = pv.read(input_png)
            pl.add_mesh(img_data, rgb=True)
            pl.add_title("ESP32-CAM Photo", font_size=9)
            pl.view_xy()
            pl.subplot(0, 1)
        ...
        pl.add_text("Drag: rotate  Scroll: zoom  O: open output folder",
        ...
        pl.add_key_event("O", open_save_folder)
        pl.reset_camera()
        pl.show(title="TripoSR ESP32-CAM - 3D Viewer")
